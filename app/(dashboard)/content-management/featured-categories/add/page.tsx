...
const AddFeatureCategoryPage = () => {
  ...
  const handleFeaturedToggle = () => {
    if (!formData.isFeatured && featuredLimitInfo && !featuredLimitInfo.canAddMore) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.isFeatured && featuredLimitInfo && !featuredLimitInfo.canAddMore) {
      const confirmed = window.confirm(
        `Maximum ${featuredLimitInfo.maxFeatured} featured categories reached. You cannot add another featured category. Would you like to save without marking as featured?`
      );
      if (confirmed) {
    ...
    try {
      console.log('🔄 Submitting feature category data:', formData);

      const response = await clientService.post<ApiResponse>('/feature-categories/', formData);
      
      if (response.data.success) {
        toast.success('Feature category created successfully!');
        router.push('/content-management');
      } else {
        throw new Error(response.data.message || 'Failed to create feature category');
      }
    } catch (error: any) {
      ...
      if (error.response?.data?.message?.includes('featured') || 
        ...
        toast.error(`Failed to create feature category: ${error.response?.data?.message || error.message}`);
      }
    } finally {
  ...
  // Check permission to add feature categories
      ...
      <div className="flex flex-col items-center justify-center min-h-[50vh]">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">
            You don't have permission to create feature categories.
          </p>
          <Link
            href="/content-management/featured-categories"
          ...
          <div className="px-8 py-6 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-gray-900">Add New Feature Category</h1>
                  <p className="mt-1 text-sm text-gray-600">
                    Create a new category that can be featured on the website
                  </p>
                </div>
              </div>
              <Link
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Enter category name (e.g., Electronics, Fashion, Home)"
                />
                {errors.name && (
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Enter any category type (e.g., Electronics, Men's Fashion, Kitchenware)"
                />
                {errors.type && (
                ...
                {/* <div className="mt-2">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {['Electronics', 'Fashion', 'Home Decor', 'Sports Equipment', 'Books', 'Beauty Products'].map((example, index) => (
                      <span 
                        key={index}
                        onClick={() => {
                          setFormData(prev => ({ ...prev, type: example }));
                          setTypeCharCount(example.length);
                        }}
                        className="inline-block px-2 py-1 text-xs bg-gray-100 text-gray-600 rounded cursor-pointer hover:bg-gray-200 transition-colors"
                      >
                        {example}
                      </span>
                    ))}
              ...
              {/* <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <p className="text-sm font-medium text-gray-700 mb-2">Tips for Great Categories:</p>
                <ul className="text-sm text-gray-600 space-y-1">
                  ...
                  <li className="flex items-center gap-2">
                    ...
                    Add descriptions to help users understand
                  </li>
                  <li className="flex items-center gap-2">
                    <Type className="w-4 h-4 text-blue-500" />
                    Choose relevant types for better organization
                  </li>
                </ul>
              </div> */}
            </div>
...
<div>
  ...
  {/* Calculate disabled state properly to avoid TypeScript error */}
  {(() => {
    // Fix: Ensure disabled prop is always boolean
    const isDisabled = Boolean(
      featuredLimitInfo && !featuredLimitInfo.canAddMore && !formData.isFeatured
    );
...
export default AddFeatureCategoryPage;