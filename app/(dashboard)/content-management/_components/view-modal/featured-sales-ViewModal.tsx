...
interface FeaturedSalesViewModalProps {
  isOpen: boolean;
  onClose: () => void;
  data: any;
}
...
interface FeaturedSaleResponse {
  success: boolean;
  message: string;
  data: FeaturedSaleItem;
}

const FeaturedSalesViewModal: React.FC<FeaturedSalesViewModalProps> = ({ 
  isOpen, 
  onClose, 
  data 
}) => {
  const [featuredSale, setFeaturedSale] = useState<FeaturedSaleItem | null>(null);
  const [isLoading, setIsLoading] = useState(true);

  // Fetch featured sale data when modal opens
  useEffect(() => {
    const fetchFeaturedSaleData = async () => {
      ...
      try {
        ...
        const saleId = data.id || data._id || (data.data && (data.data.id || data.data._id));
        
        if (!saleId) {
          console.error('❌ No featured sale ID found in data:', data);
          setIsLoading(false);
          return;
        }

        console.log('🔄 Fetching featured sale data for View modal ID:', saleId);
        
        const response = await clientService.get<FeaturedSaleResponse>(
          `/featured-sales/${saleId}`
        );
        
        console.log('✅ View Modal Featured Sale data received:', response.data);
        
        if (response.data.success) {
          setFeaturedSale(response.data.data);
        } else {
          console.error('❌ Failed to fetch featured sale data for view modal');
        }
      } catch (error) {
        console.error('❌ Error fetching featured sale for view modal:', error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const displayData = featuredSale || data;
        ...
        <div className="relative bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-gray-200">
            ...
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-xl font-bold text-gray-900">
                  Featured Sale Details
                </h2>
                <p className="text-sm text-gray-600 mt-1">
                  Complete information about this featured sale
                </p>
              </div>
              <button
                onClick={onClose}
          ...
          <div className="px-6 py-4">
            {isLoading ? (
              <div className="flex justify-center items-center py-12">
                ...
                <p className="ml-3 text-gray-600">Loading featured sale data...</p>
              </div>
            ) : (
              <div className="space-y-6">
                {/* Grid Layout for Details */}
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {/* Left Column - Sale Information & Timestamps */}
                  <div className="space-y-6">
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-sm font-medium text-gray-700">Sale Title</span>
                          </div>
                        ...
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-sm font-medium text-gray-700">Sale Subtitle</span>
                          </div>
                          <div className="p-3 bg-gray-50 rounded border border-gray-200">
                        ...
                        {displayData.tag && (
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className="text-sm font-medium text-gray-700">Sale Tag</span>
                            </div>
                            <div className="inline-flex items-center px-3 py-1.5 bg-orange-100 text-orange-800 rounded-lg text-sm font-medium border border-orange-200">
                              {displayData.tag}
                            </div>
                            <p className="text-xs text-gray-500 mt-1">
                              Optional sale identifier tag
                            </p>
                          </div>
                        )}
                      </div>
                          ...
                          <div className="p-3 bg-gray-50 rounded border border-gray-200">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-3">
                                <div className="inline-flex items-center px-4 py-2 bg-orange-600 text-white text-sm font-medium rounded shadow-sm">
                          ...
                          <div className="p-3 bg-gray-50 rounded border border-gray-200">
                            <div className="flex items-center justify-between">
                              <a
                                ...
                                className="text-orange-600 hover:text-orange-800 hover:underline truncate text-sm flex-1 mr-2"
                    ...
                    <div className="bg-linear-to-r from-orange-50 to-amber-50 rounded-lg p-5 border border-orange-200 shadow-sm">
                      <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
                          ...
                          {displayData.image ? (
                            <div className="relative w-full h-full">
                              <Image
                                ...
                                alt={displayData.title || "Featured Sale"}
                                fill
                          ...
                          <div className="absolute inset-0 flex flex-col justify-end p-5">
                            <div className="text-white w-full">
                              <h3 className="text-xl font-bold mb-2 leading-tight drop-shadow-lg">
                                {displayData.title || 'Featured Sale'}
                              </h3>
                              ...
                              <div className="flex flex-wrap items-center gap-3">
                                <a
                                  ...
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="inline-flex items-center px-4 py-2 bg-linear-to-r from-orange-600 to-orange-700 hover:from-orange-700 hover:to-orange-800 text-white font-medium rounded-lg transition-all duration-300 shadow-xl hover:shadow-2xl hover:-translate-y-0.5 active:translate-y-0"
                                >
                                  <span className="mr-2">{displayData.buttonText || 'Shop Now'}</span>
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            <div className="text-gray-700">
                              ...
                              {displayData.image ? (
                                <span className="text-orange-600 font-medium">Cloudinary Storage</span>
                              ) : (
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-lg border border-gray-200 p-5 shadow-sm">
                      ...
                      <div className="space-y-3">
                        {/* Sale ID */}
                        <div>
                          <p className="text-sm font-medium text-gray-700 mb-1">Sale ID</p>
                          <div className="p-3 bg-gray-50 rounded border border-gray-200">
                            <div className="flex items-center justify-between">
                              ...
                              <button
                                ...
                                className="ml-2 text-xs text-orange-600 hover:text-orange-800 whitespace-nowrap"
                              >
                                Copy ID
                              </button>
                            </div>
                            <p className="text-xs text-gray-500 mt-2">
                              Unique identifier for this sale in the database
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-3 pt-3">
                          ...
                          <div className="p-2 rounded text-center bg-orange-50 border border-orange-200">
          ...
          <div className="sticky bottom-0 bg-white px-6 py-4 border-t border-gray-200 rounded-b-lg">
            <div className="flex justify-center">
              <button
                onClick={onClose}
                className="px-6 py-2 bg-orange-600 text-white text-sm font-medium rounded-md hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 transition-colors shadow-sm"
              >
                Close
              </button>
            </div>
...
export default FeaturedSalesViewModal;