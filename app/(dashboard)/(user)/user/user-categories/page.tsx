...
const CategoriesPage = () => {
  ...
  useEffect(() => {
    console.log('🔍 DEBUG - Current permissions in CategoriesPage:', {
      permissions,
      ...
      user_categories_view: permissions['user_categories.view'],
      allTruePermissions: Object.keys(permissions).filter(k => permissions[k])
    });
    
    // Also check localStorage
    const storedPermissions = localStorage.getItem('userPermissions');
    console.log('🔍 Stored permissions in localStorage:', storedPermissions);
  }, [permissions]);
  ...
  // Handle edit category - Updated with permission helper
  const handleEditCategory = (category: any) => {
  ...
  // Handle delete category confirmation - Updated with permission helper
  const handleDeleteCategory = async () => {
    if (!deletingCategory) return;
  ...
  // Update the handleStatusChange function - Updated with permission helper
  const handleStatusChange = async (category: any, status: 'active' | 'inactive') => {
    // Check if user has edit permission
    if (!hasPermission('user_categories.edit')) {
  ...
  // Handle Add Category button click - Check permission before adding
  const handleAddCategory = () => {
  ...
  const columns = useCategoryColumns({
    ...
    onStatusChange: handleStatusChange,
    onView: handleViewCategory,
    // Pass permissions to the column hook if it supports it
  ...
  // Check if user has view permission for categories - Updated with permission helper
  if (!hasPermission('user_categories.view')) {
    return (
  ...
  return (
    <div>
      <DataGridWrapper
        ...
        addButtonText="Add Category"
        addButtonOnClick={handleAddCategory}
        addButtonPermission="user_categories.create"
        hasExportButton={true}
        onExport={() => console.log('Export categories')}
        allPermissions={permissions}
        enableRowActions={true}
        editPermission="user_categories.edit"
        deletePermission="user_categories.delete"
      />
